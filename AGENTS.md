...
- Startup requests run in the same process as the desktop window, so long synchronous work freezes the app before it paints ("Books Selection (Не отвечает)" with a blank content area). Keep them cheap: `GET /api/embedding-status` counts readiness with SQL (`countChunksWithCurrentEmbedding` plus a bounded 20-row validity sample, falling back to `countReadyEmbeddings` when a sampled vector is malformed) instead of parsing every stored vector — per-row `JSON.parse` over 20 000 vectors took ~6 s and was the visible startup freeze. Never put per-row vector parsing, joined `ORDER BY`, or dimension scans over `chunk_embeddings` on a startup path; the dimension-filtered `countReadyEmbeddings` belongs to the Ask path, where the expected dimension is known.
...
- `src/ask.js`: readiness/provider orchestration for bounded model-guided Ask; it never sends the full library to a model.
- `src/askResearch.js`: plan → retrieve → check → optional refinement → final answer, with query/context/call caps, cancellation, cited-only recommendation cards, and hash-linked observations in `derived_facts`. Chat and embedding providers stay separate. Cached observations are leads, not authoritative proof.
- Preparation adds section paths, source ordering and previously omitted supplemental/notes text; unchanged legacy chunks and embeddings survive context backfill. `getChunkContext()` returns the target first and bounded adjacent source context. Index completeness is not whole-cycle plot verification.
