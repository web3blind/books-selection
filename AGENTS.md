...
## Structure
- `src/server.js`: локальный HTTP server, раздача `public/index.html`, API `GET /api/books`, `POST /api/index`, `GET /api/search`, `GET /api/ask`.
...
- `src/providerClient.js`: mockable OpenAI-compatible chat completion scaffold с injectable `fetchImpl`; не логирует и не возвращает секреты.
