...
Статус: `v0.3.12` опубликована и проверена. Группировка найденных вариантов Ask по циклам реализована в `main` и ещё не выпущена. Следующие согласованные этапы: избранное и рейтинг циклов по ответам Ask, затем учёт прочитанных циклов, отметка «Цикл не закончен» и ручная проверка продолжений через публичные страницы Author.Today. Hermes provider transport Денис реализует отдельно; до появления транспорта Hermes остаётся внутренним scaffold и не показывается как рабочий выбор в UI.
