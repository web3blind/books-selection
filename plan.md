...
Статус: `v0.4.1` опубликована 2026-09-17 (`https://github.com/web3blind/books-selection/releases/tag/v0.4.1`, коммит `fc4d36f`): общая кнопка проверки всех незаконченных циклов и устранение подвисания после запуска. Предыдущая версия — `v0.4.0` (`df6b1a0`, группировка ответов Ask по циклам, избранное с рейтингом, отметки прочитанных циклов, проверка продолжений Author.Today, запоминание языка). Hermes provider transport Денис реализует отдельно; до появления транспорта Hermes остаётся внутренним scaffold и не показывается как рабочий выбор в UI.
