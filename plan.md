...
Статус: группировка найденных вариантов Ask по циклам, избранное с рейтингом и отметки прочитанных циклов реализованы в `main` и ещё не выпущены. Осталось по плану: авторы, проверка продолжений через Author.Today и экспорт отметок; затем релиз. Hermes provider transport Денис реализует отдельно; до появления транспорта Hermes остаётся внутренним scaffold и не показывается как рабочий выбор в UI.
...
- Извлечение авторов из FB2 и поиск по автору.
- Привязка публичной страницы цикла Author.Today, ручная проверка продолжений и раздел «Есть продолжение».
- Экспорт и восстановление отметок.
