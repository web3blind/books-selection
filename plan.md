...
Статус: `v0.3.12` опубликована и проверена. Следующие согласованные этапы: сначала избранное и рейтинг циклов по запросам, затем учёт прочитанных циклов, отметка «Цикл не закончен» и ручная проверка продолжений через публичные страницы циклов Author.Today. Hermes provider transport Денис реализует отдельно; до появления транспорта Hermes остаётся внутренним scaffold и не показывается как рабочий выбор в UI.
...
- В расчёт входит только попадание в топ-5 видимой выдачи циклов; позиции ниже пятой баллов не начисляют.
...
1. После поиска сервер берёт порядок циклов, который реально показан пользователю: одна карточка на цикл, как на главной.
...
6. Сортировка «Избранного» по умолчанию: рейтинг по убыванию, затем `leader_count`, затем имя цикла; доступна сортировка по имени и автору.
7. Агрегированный балл не хранится: он вычисляется из hits-таблицы, чтобы значение не расходилось с историей.

Отклонённые варианты: reciprocal rank (`1/r`) как основная шкала — менее понятен пользователю при отображении; накопление без дедупликации — один повторяемый запрос начинал бы управлять рейтингом.

### Data model

- `cycle_favorites`: `cycle_key`, `added_at`.
...
- Кнопки «В избранное» / «Убрать из избранного» на карточке цикла с понятным доступным названием и состоянием.
- Отдельная страница «Избранное»: имя цикла, автор, рейтинг, «лидер в N запросах», раскрываемый список запросов с позициями.
- Переключатели сортировки, кнопки очистки истории и пустое состояние с объяснением, как считается рейтинг.
...
- UI static-проверки и browser smoke: отметить цикл, выполнить запрос, увидеть рост рейтинга и список запросов; консоль без ошибок.
- `npm test`, `git diff --check`, `npm audit --omit=dev`.

### Open questions

- Учитывать ли выдачу Ask по циклам так же, как обычный поиск, или только основную выдачу.
- Учитывать ли запросы, сделанные до добавления цикла в избранное (по умолчанию нет).
- Нужен ли ручной подъём/закрепление поверх автоматического рейтинга.
...
- `outcome`: пользователь одной кнопкой отмечает цикл прочитанным, при необходимости ставит отметку «Цикл не закончен», находит прочитанное по названию цикла или автору и вручную проверяет привязанные циклы на появление продолжений.
- `verification`: отметки переживают повторное индексирование и перезапуск; прочитанные можно скрыть из основного поиска; раздел «Прочитанное» фильтруется по циклу/автору и отдельно показывает незаконченные циклы; добавленный на Author.Today `work_id` и смена статуса цикла обнаруживаются без ложного срабатывания на перестановку книг.
...
- `boundaries`: `src/searchSchema.js`, совместимая миграция SQLite, извлечение авторов из FB2, indexer/server API, новый узкий модуль Author.Today, `public/index.html`, focused tests, backup/export и документация. AI/provider workflow не меняется.
...
2. **Authors and stable cycle identity**
   - Расширить FB2 parser извлечением авторов и сохранить авторов всех книг цикла при полном индексировании.
   - Зафиксировать тестами нормализацию имени, несколько авторов, отсутствие автора и одноимённые циклы в разных корнях.
...
4. **Accessible cycle-only UI**
   ...
   - Добавить раздел «Прочитанное», поиск по циклу/автору, подраздел «Незаконченные циклы» и переключатель «Скрывать прочитанные» в основном поиске.
...
7. **Backup and restore**
   - Экспортировать пользовательские отметки, авторов-привязки и Author.Today snapshots в версионированный JSON без embeddings, книжных текстов и credentials.
   - Импорт выполнять через предварительную проверку и не перезаписывать неоднозначные циклы автоматически.
...
8. **Integration and release gate**
   ...
   - Перед публикацией проверить миграцию на копии существующей базы, экспорт/восстановление отметок и все desktop artifacts.
