...
Статус: группировка найденных вариантов Ask по циклам и избранное с рейтингом циклов реализованы в `main` и ещё не выпущены. Следующий согласованный этап: учёт прочитанных циклов (отметка «Цикл не закончен» и ручная проверка продолжений через публичные страницы Author.Today), затем релиз. Hermes provider transport Денис реализует отдельно; до появления транспорта Hermes остаётся внутренним scaffold и не показывается как рабочий выбор в UI.
