...
Статус: группировка найденных вариантов Ask по циклам, избранное с рейтингом и отметки прочитанных циклов реализованы в `main` и ещё не выпущены. Осталось по плану: проверка продолжений через Author.Today; затем релиз. Hermes provider transport Денис реализует отдельно; до появления транспорта Hermes остаётся внутренним scaffold и не показывается как рабочий выбор в UI.
