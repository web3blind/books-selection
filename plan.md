...
Статус: весь согласованный объём `v0.4.0` реализован в `main` и ещё не выпущен: группировка ответов Ask по циклам, избранное с рейтингом, отметки прочитанных циклов и ручная проверка продолжений через Author.Today. Осталось собрать и опубликовать релиз. Hermes provider transport Денис реализует отдельно; до появления транспорта Hermes остаётся внутренним scaffold и не показывается как рабочий выбор в UI.
