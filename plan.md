...
- Settings/config UI increment delivered: `src/appConfig.js` manages `~/.books-selection/config.json` (or `BOOKS_SELECTION_CONFIG_PATH`) without storing raw API keys, `GET/POST /api/config` read/write normalized settings, first launch opens Settings when `booksRoot`/`dbPath` are missing, and the main/index/ask/embedding APIs use saved root/db/provider overrides by default.
