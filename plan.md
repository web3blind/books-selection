...
Статус: `v0.3.12` опубликована и проверена. Следующие согласованные этапы: сначала группировка найденных вариантов Ask по циклам, затем избранное и рейтинг циклов по ответам Ask, затем учёт прочитанных циклов, отметка «Цикл не закончен» и ручная проверка продолжений через публичные страницы Author.Today. Hermes provider transport Денис реализует отдельно; до появления транспорта Hermes остаётся внутренним scaffold и не показывается как рабочий выбор в UI.
...
### Текущее состояние (факт по коду)
...
- UI static и browser smoke: несколько книг одного цикла дают один блок с вложенным списком.
- `npm test`, `git diff --check`.

### Open question

- Книги внутри блока показывать сразу или под раскрываемым `details`/`summary`.
