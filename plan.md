...
Статус: исправления по аудиту реализованы для `v0.3.9`, учли замечания двух раундов независимого ревью и прошли `178/178` тестов, browser/API/Electron smoke, release-build и проверку архивов. Публикация GitHub Release `v0.3.9` с Linux, Windows и macOS артефактами одобрена Денисом. Обновление защищённого `AGENTS.md` не входило в реализацию; код, README и тесты используют актуальные POST-контракты. Hermes provider transport Денис реализует отдельно; до появления транспорта Hermes остаётся внутренним scaffold и не показывается как рабочий выбор в UI.
