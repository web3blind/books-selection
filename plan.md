...
Статус: `v0.4.0` опубликована 2026-09-17 (`https://github.com/web3blind/books-selection/releases/tag/v0.4.0`, коммит `df6b1a0`): группировка ответов Ask по циклам, избранное с рейтингом, отметки прочитанных циклов, ручная проверка продолжений через Author.Today и запоминание языка интерфейса. Hermes provider transport Денис реализует отдельно; до появления транспорта Hermes остаётся внутренним scaffold и не показывается как рабочий выбор в UI.
